...
export class AccountService {
  ...
  checkBicService(bicNumber:String){
    const url =
    'http://localhost:8080/bankDetails/' + bicNumber ;
  const data = {
    accNumber: bicNumber ,
  };
  ...
  transferTypeService(transferType:String){
    const url =
    'http://localhost:8080/transferTypeDetails/' + transferType;
  ...
  messageCodeService(msgCode:String){
    const url =
    'http://localhost:8080/messageCodeDetails/' + msgCode;
  ...
  transferService(tData:any){
    const url =
    'http://localhost:8080/addTransactionDetails/';
  // const data = {
...
transactionidService(transactionid:Number){
  const url =
    'http://localhost:8080/transactionDetails/' + transactionid;
  ...
  activate(accountNo:Number){
    const url =
      'http://localhost:8080/customerDetails/' + accountNo ;
    const data = {
      accNumber: accountNo ,
    };
    return this.httpClient.get<any>(url);
  ...
  sanctionService(accHolderName:String){
    const url =
    'http://localhost:8080/checkSDNList/' + accHolderName ;
  const data = {
